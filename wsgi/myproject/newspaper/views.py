...
def lifestyleRedirect( request ) :
    ## try get all the articles of type TECHNOLOGY
    try :
        articleList = Article.objects.filter( articleType  = 'lifestyle' )
    ...
    ## no articles of type TECHNOLOGY
    except Exception :
        ## check if not logged in
        if ( 'username' not in request.session ) :
            return render( request , 'newspaper/lifestyle.html' , {} )
...
def sportRedirect( request ) :
    ## try get all the articles of type SPORT
    try :
        articleList = Article.objects.filter( articleType  = 'sport' )
    ...
    ## no articles of type SPORT
    except Exception :
        ## check if not logged in
        if ( 'username' not in request.session ) :
            return render( request , 'newspaper/sport.html' , {} )
...
def politicsRedirect( request ) :
    ## try get all the articles of type POLITICS
    try :
        articleList = Article.objects.filter( articleType  = 'politics' )
    ...
    ## no articles of type POLITICS
    except Exception :
        ## check if not logged in
        if ( 'username' not in request.session ) :
            return render( request , 'newspaper/politics.html' , {} )
...
def businessfinanceRedirect( request ) :
    ## try get all the articles of type BUSINESS + FINANCE
    try :
        articleList = Article.objects.filter( articleType  = 'businessfinance' )
    ...
    ## no articles of type BUSINESS + FINANCE
    except Exception :
        ## check if not logged in
        if ( 'username' not in request.session ) :
            return render( request , 'newspaper/businessfinance.html' , {} )
...
def technologyRedirect( request ) :
    ## try get all the articles of type TECHNOLOGY
    try :
        articleList = Article.objects.filter( articleType  = 'technology' )
    ...
    ## no articles of type TECHNOLOGY
        ...
        ## check if not logged in
        if ( 'username' not in request.session ) :
            return render( request , 'newspaper/technology.html' , {} )
...
def updateDetails( request ) :
    ...
    if ( request.method == 'POST' ) :
        ...
        ## validate inputs
        nameNotFilledIn      = False
        telephoneNotFilledIn = False

        ## check if name is in correct format + != empty
        if ( fName.isspace() or not fName ) :
            nameNotFilledIn  = True

        ## check if telephone is in correct format + != empty
        if ( isTelephoneNumber is None ):
            telephoneNotFilledIn = True

        ## check if both have been entered properly
        if ( not nameNotFilledIn and not telephoneNotFilledIn ) :
        ...
        ## only telephone num. enetred properly
        elif ( not nameNotFilledIn ) :
            user.first_name  = fName
            user.save()
            context = {
        ...
        ## only name num. enetred properly
        elif ( not telephoneNotFilledIn ) :
            user.phoneNumber = pNumber
            user.save()
            context = {

                          'success'    : 'telephoneonly',
                          'pNumber'    : pNumber

                      }
            return JsonResponse( context )

        ## if both tests fail, then return false
        else :
            context = {
...
def getLikes( request , articleID ) :
    ...
    if ( 'username' in request.session ) :
        ...
        ## user hasn't liked/disliked before
        except Exception :
            context = {
